...
-- Dataset columns: OrderID, Month, Quarter, State, CustomerName,
...
WITH customer_agg AS (
    SELECT
        ...
        MAX(Month) AS last_month_active
    FROM ecommerce_data
...
WITH payment_metrics AS (
    SELECT
        ...
        SUM(Amount) AS total_sales,
        SUM(Profit) AS total_profit,
        ROUND(100.0 * COUNT(DISTINCT OrderID) / (SELECT COUNT(DISTINCT OrderID) FROM ecommerce_data), 2) AS pct_of_orders
    FROM ecommerce_data
